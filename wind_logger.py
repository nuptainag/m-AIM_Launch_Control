...
try:
    ...
    if not file_exists:
        csv_writer.writerow(['Timestamp', 'Wind Speed (m/s)', 'Raw Value'])
        ...
        if ser.in_waiting > 0:
            ...
            if len(response) >= 7:
                ...
                print(f"[{timestamp.strftime('%H:%M:%S')}] "
                      f"Wind Speed: {wind_speed:5.1f} m/s | "
                      f"Avg: {current_avg:5.1f} | "
                      f"Min: {current_min:5.1f} | "
                      f"Max: {current_max:5.1f} | "
                      f"Samples: {reading_count}")
                
                # Alert for high wind speeds
                if wind_speed > 10.0:
                    print(f"  ⚠️  HIGH WIND ALERT: {wind_speed} m/s")
    ...
    if readings:
        ...
        print(f"  Average wind speed: {sum(readings)/len(readings):.2f} m/s")
        print(f"  Maximum wind speed: {max(readings):.2f} m/s")
        print(f"  Minimum wind speed: {min(readings):.2f} m/s")
